...
- **Current Stage**: 개선 — CLI `caduceus chat`을 sessions/chat/stream 으로 마이그레이션 (2026-07-06, 경량 사이클). web과 동일한 sessions 백엔드 재사용(별도 재구현 없음). 전송만 교체(`chat.py`): runs(GET messages+conversation_history+POST v1/runs+GET events) → `POST api/sessions/{id}/chat/stream {message}` 1-콜 named SSE. run_id는 run.started에서 캡처(stop-전-도착 pending 처리). **Q1=B3 reasoning 미표시**(실측: sessions도 reasoning이 content 뒤 도착, 콘솔 재배치 불가)/Q2=A 웹 hydrate-outage 수정(별도 커밋)/Q3=A `_history` 제거. 세션 해석(`resolve_session`)·`--session`/`--new`·`iter_sse`·`agent_api_stream`는 이미 충족→무변경. **적대적 리뷰(5차원×refuter, 14에이전트) 반영**: tool 실패 페어링 회귀(#1)를 `run.completed.messages`(#34703) 정확 스코핑+`!=` 가드로 수정(멀티턴 오염 해소·완전 1-콜) + 커버리지 4건. 검증 **ruff/mypy clean, pytest 308, web tsc/eslint/vitest 110·build·E2E 20/20**. docs: `construction/cli-chat-sessions-migration/`. **커밋 진행(CLI / 웹 hydrate 2커밋).**
- **직전 Stage**: 개선 — per-agent Chat UI를 sessions/chat/stream 으로 마이그레이션 (2026-07-05, 경량 사이클). 선행: hermes fork 패치(PR #58856, `/api/sessions/{id}/chat/stream`에 approval/stop 이식) → 로컬 `/home/beom/.hermes/hermes-agent` editable를 feat 브랜치로 재설치 + 라이브 HTTP 스파이크(실제 _run_agent+approval.py+/v1/runs 왕복) [A]approval/[B]stop 실증. 게이트웨이 3개(test/2/3) 재시작 완료(패치 라이브). **Requirements APPROVED (2026-07-05)** — Q1=A(per-agent만 전환, 룸 runs 유지 직교)/Q2=A(미사용 runs 클라 제거)/Q3=A(전송만, 이미지 후속)/**Q4=B(라이브 thinking 표시 FR-9)**/Q5=A. 스코프: `web/src/`(ChatView·agentApi·client·types·조건부 transcript) — 백엔드/proxy/hermes 무변경. **Code Gen(S1~S10) 완료 + ultracode 적대적 리뷰(5차원×refuter, 15에이전트) 반영**: 전환(runs→sessions/chat/stream 1-콜, named SSE, 네이티브 히스토리, 라이브 thinking) + 리뷰 수정 3건(Stop 레이스 pendingStopRef / 라이브 hex redact / sendApproval .catch) + 커버리지 3건. 검증 **tsc·eslint clean, vitest 110/110(1 기존 flaky), build(web_dist 갱신), E2E 19/19**. docs: `construction/chat-sessions-migration/`(requirements/requirements-questions/plan/code-summary). **Next: Code Gen 승인 게이트(미커밋).**
