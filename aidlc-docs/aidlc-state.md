...
- **Current Stage**: 개선 — 브라우저 로컬 접근 opt-in (`allow_private_urls`) (2026-07-03, 경량 사이클). 배경: terminal host fix 후 curl은 localhost 도달하나 hermes browser는 SSRF 가드(url_safety)가 loopback/사설 기본 차단("Blocked: URL targets a private or internal address"). 결정: per-agent opt-in, 기본 off, `network_mode`와 동일 배선. 변경: AgentSpec.allow_private_urls / managed_config `security.allow_private_urls` 렌더 / CLI `--allow-private-urls` + status 표시 / web(types·forms·생성폼 체크박스·detail 표시). **+ 생성 후 편집**: `GET/PUT /api/agents/{name}/allow-private-urls`(승인 모드 패턴, spec 갱신+config 재렌더+재시작 배너) / client 메서드 / SettingsTab 토글. `network_mode`는 컨테이너 재생성 필요라 제외(읽기전용 유지). 테스트: strategies·render 단위+PBT·web forms·daemon_asgi 통합. ruff·mypy(43)·pytest **496** / web typecheck·eslint·vitest **73**·**e2e 14/14**·build(web_dist 갱신). 실기 실증: 렌더 config를 hermes `read_raw_config`가 읽어 `is_safe_url(localhost)` True/False 토글 확인. **코드 승인 대기(승인 시 커밋)**. docs: `construction/browser-private-urls/`.
