...
- **Current Stage**: 개선 — Observability 전면 재설계 (2026-07-04, 경량 사이클). 사전 스파이크: hermes dashboard 실기동 검증 — analytics(`/api/analytics/usage|models`, `/api/sessions/stats`)는 전부 세션 DB SQL 집계이며 원천 per-session 행(api_call_count/토큰6종/cost/tool_call_count/message_count/timestamps/model/source)은 이미 relay 중인 api_server `/api/sessions`가 제공(대시보드 상주 불필요, ~107MB×N 회피). Requirements v1.1 **APPROVED & 커밋** — IA: 전용 Observability 메뉴, **Total**(플릿 롤업+agent 비교) + **Agent**(metric 패널 내 session narrow-down; latency/errors는 gateway가 세션 id 미보유라 agent 레벨 한정), Q2=C 하이브리드(데몬 집계 엔드포인트+링 노출), Q3=C 게이트웨이 인메모리 확장만(휘발 유지·링 대폭 확장·초/분 롤업·메모리 캡), Q4=A 세션버킷, Q5=B 24h/7d/30d+Live, Q6=B Rich 지표(per-tool breakdown 제외·tool_call_count 합계만), Q7=A 폴링. v1.1 증보: 상용 솔루션급 UI 품질 기준(§11). docs: `construction/observability-redesign/`. **다음: Code Generation Plan 승인 게이트**.
