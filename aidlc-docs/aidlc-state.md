...
- **Current Stage**: 개선 — 에이전트 터미널 host 네트워크 (gateway `TERMINAL_*` env 주입) (2026-07-03 시작). 범위: caduceus가 gateway spawn 시 spec 파생 `TERMINAL_*` env를 주입해 `default` 터미널 샌드박스가 `spec.network_mode`(host 등)로 뜨게 함. 근본원인: hermes terminal_tool은 `TERMINAL_*` env만 읽는데 caduceus가 gateway에 env 미주입 → default가 bridge로 뜸. Requirements APPROVED (Q1=A 네트워크만 / Q2=A network_mode 반영 / Q3=A config 유지+env 주입 / `prompt-backend-probe` 정리는 후속 분리). Plan(S1~S7) APPROVED → **Part 2 Generation 완료**(S1~S7 전체 [x]). 변경: render.terminal_env 순수함수 / hermes_adapter.gateway_env / process_manager env 스레딩(재시작 안전) / provisioner·lifecycle·daemon 3개 호출부 주입 / 테스트(PBT 포함). ruff·mypy(43)·pytest **494 passed**(통합 4 deselected). 실기 실증: 주입 env를 설치된 hermes `_get_env_config()`가 docker+`--network=host`+persistent로 해석 확인. **코드 승인 대기(승인 시 커밋)**. docs: `construction/terminal-host-network/` (code-summary.md 포함).
