...
- [ ] **S1 — TrafficStats 확장** (`caduceus/proxy/traffic.py`)
...
- [ ] **S2 — 순수 세션 집계 모듈** (`caduceus/control/observability.py` 신규)
...
- [ ] **S3 — 데몬 fan-out 수집기** (`caduceus/control/observability.py` 내 async 부)
...
- [ ] **S4 — 관측 API 엔드포인트** (`caduceus/control/api.py`)
...
- [ ] **S5 — 계약 타입·클라이언트** (`web/src/lib/types.ts`, `web/src/api/client.ts`)
...
- [ ] **S6 — 차트 키트 확장** (`web/src/components/charts.tsx` 또는 `charts/` 분리)
...
- [ ] **S7 — Observability 페이지** (`web/src/pages/observability/` 신규)
