...
## C7. Service Manager Adapter — `caduceusd` 내부 모듈

**Purpose**: OS 서비스 통합 (Linux systemd user unit / macOS launchd) — 감독(재시작)은 OS에 위임 (AD-5).

**Responsibilities**:
- 에이전트별 hermes gateway 서비스 등록/해제/start/stop/status (hermes 네이티브 per-profile 서비스 경로 우선 사용, 부재 시 표준 unit/plist 생성)
- `caduceusd` 자신의 서비스 등록 (`caduceus init --service`)
- 플랫폼 추상화: systemd/launchd 두 백엔드 (N9)

**Interfaces**: in-proc API; 하위 subprocess(systemctl --user / launchctl)
