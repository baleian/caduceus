...
| **F3** | 환경 자체의 분리 (핵심) | 한 에이전트가 시스템 패키지(예: `apt-get`)를 설치하면 그 에이전트에서는 지속되고, 다른 에이전트에는 전혀 영향이 없어야 한다 |
...
2. 한 에이전트에서 `apt-get install` 등 환경 변경을 수행해도 다른 에이전트 환경은 불변이고, 해당 에이전트 재시작 후에도 지속된다 (F3)
