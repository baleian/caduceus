...
**Version**: 1.3 (F3 확정: rootless docker 모델 — 2026-07-03)
**Date**: 2026-07-02
...
| **F3** | 환경 자체의 분리 (핵심) | 한 에이전트가 환경에 설치한 도구(apt-get 시스템 패키지 포함)는 그 에이전트에서는 지속되고, 다른 에이전트에는 전혀 영향이 없어야 한다. *(v1.3 확정: 샌드박스는 **rootless docker 데몬**(preflight 강제) 위에서 container root로 실행 — 컨테이너 안에서는 apt-get 등 완전한 root 권한, 호스트에서는 모든 산출물(workspace·샌드박스)이 데몬 사용자 소유. 소유권 workaround(스크럽·tmpfs·setgid·umask 주입) 전부 불필요·제거. `docker.host` 설정으로 rootless 소켓 지정. 결정 2026-07-03, sandbox-ownership-question.md 최종 결정 참조)* |
...
2. 한 에이전트에서 apt-get/pip/npm 등 환경 변경을 수행해도 다른 에이전트 환경은 불변이고, 해당 에이전트 재시작 후에도 지속된다 (F3, v1.2 재개정)
