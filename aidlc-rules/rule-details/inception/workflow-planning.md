...
- **Deployment model changes** (serverless↔container, VM↔serverless, etc.)

#### Related Component Identification
For transformations, identify:
- **Infrastructure code** that needs updates
- **Infrastructure-as-Code** definitions requiring changes
...
- **Infrastructure-as-Code** packages requiring updates
...
- **Deployment model**: serverless→containers, VM→managed runtime, etc.
- **Networking**: networks, security/firewall rules, load balancers
...
- **Monitoring**: metrics platform, custom metrics, dashboards
