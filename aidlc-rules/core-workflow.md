...
**CRITICAL**: When performing any phase, you MUST read and use relevant content from rule detail files. These files live in the `rule-details/` directory alongside this workflow file.

All subsequent rule detail file references (e.g., `common/process-overview.md`, `inception/workspace-detection.md`) are relative to that `rule-details/` directory.
